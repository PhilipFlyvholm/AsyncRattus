...
import WidgetRattus.Signal hiding (buffer, current, future, interleave, interleaveAll, map, scan, switchR, switchS, trigger, triggerM)
...
stepper initial event =
  Beh (\_ -> K initial ::: delay (adv (aux initial event)))
  where
...
trigger :: (Stable b) => Box (a -> b -> c) -> Ev a -> Beh b -> C (Ev c)
trigger f e (Beh b) = do
  t <- time
  let b' = b t
  return $ EvSparse (trig f e b')
  where
    trig :: (Stable b) => Box (a -> b -> c) -> Ev a -> SBeh b -> O (Sig (Maybe' c))
    trig f' (EvDense as) (b ::: bs) =
      delayC $
        delay
          ( let d = select as bs
             in ( do
                    ...
                    return
                      ( case d of
                          Fst (a' ::: as') bs' -> Just' (unbox f' a' (apply b t)) ::: trig f' (EvDense as') (b ::: bs')
                          Snd as' bs' -> Nothing' ::: trig f' (EvDense as') bs'
                          Both (a' ::: as') (b' ::: bs') -> Just' (unbox f' a' (apply b' t)) ::: trig f' (EvDense as') (b' ::: bs')
                      )
                )
          )

-- trigger :: forall a b c. (Stable b) => Box (a -> b -> c) -> Ev a -> Beh b -> Ev c
-- trigger f event behaviour =
--   let d =
--         ( delayC
--             ( delay
--                 ( do
--                     d' <- run f event behaviour
--                     return (adv d')
--                 )
--             )
--         )
--    in EvSparse d
--   where
--     run :: forall a b c. (Stable b) => Box (a -> b -> c) -> Ev a -> Beh b -> C (O (Sig (Maybe' c)))
--     run f' event (Beh behaviour) = do
--       t <- time
--       let b' = behaviour t
--       return $ withTime (delay (\t -> let g = adv (trig f' event) in g b' t))
--       where
--         trig :: forall a b c. (Stable b) => Box (a -> b -> c) -> Ev a -> O (SBeh b -> Time -> Sig (Maybe' c))
--         trig f' (EvDense as) =
--           delay
--             ( \(b ::: bs) t ->
--                 case select as bs of
--                   Fst (a' ::: as') bs' -> Just' (unbox f' a' (apply b t)) ::: delay (let g' = adv (trig f' (EvDense as')) in g' (b ::: bs') t)
--                   Snd as' bs' -> Nothing' ::: delay (let g' = adv (trig f' (EvDense as')) in g' bs' t)
--                   Both (a' ::: as') (b' ::: bs') -> Just' (unbox f' a' (apply b' t)) ::: delay (let g' = adv (trig f' (EvDense as')) in g' (b' ::: bs') t)
--             )

-- trig f' (EvSparse as) (Beh bs) =
--   delayC $
--     delay
...
--                 choice = select as bs
--              in ( ( case choice of
--                       Fst (a' ::: as') bs' -> Just' (unbox f' a' (apply b t)) ::: trig f' (EvDense as') (Beh (\_ -> b ::: bs'))
--                       Snd as' bs' -> Nothing' ::: trig f' (EvDense as') (Beh (\_ -> bs'))
--                       Both (a' ::: as') (b' ::: bs') -> Just' (unbox f' a' (apply b' t)) ::: trig f' (EvDense as') (Beh (\_ -> b' ::: bs'))
--                   )
--                 )
--         )
--     )
...
switchS (Beh as) d =
  ...
  where
    run :: (Stable a) => SBeh a -> O (a -> SBeh a) -> SBeh a
    run (x ::: xs) d =
      let rest =
            delayC
              ( delay
...
switchSM (Beh as) d =
  ...
  where
    run :: (Stable a) => SBeh a -> O (Maybe' (a -> SBeh a)) -> SBeh a
    run (x ::: xs) d =
      let rest =
            delayC
              ( delay
